  ...
  return items.map((item, index) => {
    ...
    const itemClassNames = isSubItem
      ? `mt-0.5 text-link dark:text-darklink !hover:bg-transparent ${
          isSelected ? '!bg-transparent !text-[#0CCF0E]' : ''
        } !px-1.5`
      : `mt-0.5 text-link dark:text-darklink ${
          isSelected
            ? '!bg-[#0CCF0E] !text-white rounded-md'
            : 'hover:bg-lightprimary/20 rounded-md'
        }`
...
const SidebarLayout = ({ onClose }: { onClose?: () => void }) => {
  ...
  if (!mounted) {
    return (
      <div className='fixed left-0 top-0 border-r border-border dark:border-darkborder bg-white dark:bg-dark z-10 h-screen w-[270px]' />
    )
  }
