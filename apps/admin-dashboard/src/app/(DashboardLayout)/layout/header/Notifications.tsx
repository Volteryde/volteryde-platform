...
import { Bell, UserPlus, AlertCircle, CheckCircle, Info, RefreshCw } from "lucide-react";
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <div className="relative cursor-pointer">
            ...
            {unread > 0 && (
              <span className="rounded-full absolute -end-[6px] -top-[5px] h-2 w-2 bg-primary" />
            )}
          </div>
        </DropdownMenuTrigger>
        ...
        <DropdownMenuContent
          align="end"
          className="w-screen sm:w-[340px] py-4 rounded-sm border border-ld"
        >
          {/* Header */}
            ...
            {loading ? (
              ...
              notifications.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className={`flex items-start gap-3 px-5 py-3 hover:bg-muted/50 transition-colors cursor-default ${isUnread ? "bg-muted/30" : ""}`}
                  >
