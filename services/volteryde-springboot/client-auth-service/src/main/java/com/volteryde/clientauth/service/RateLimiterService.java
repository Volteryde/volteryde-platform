 ...
 * 4. Per IP: If same IP triggers limit again after 2-hour window, extend block
 * to 100 hours
 * 
 * Uses in-memory storage (replace with Redis in production for distributed
 * setup)
 */
@Service
public class RateLimiterService {
    ...
    public boolean isOtpAllowed(String phone, String ipAddress) {
        ...
        if (phoneMinuteEntry != null && !phoneMinuteEntry.isExpired()
                && phoneMinuteEntry.getCount() >= OTP_MAX_PER_MINUTE) {
    ...
    public void checkAndRecordOtp(String phone, String ipAddress) {
        ...
        if (isIpBlacklisted(ipAddress)) {
            ...
            throw new RuntimeException(
                    String.format(
                            "Your IP has been temporarily blocked due to suspicious activity. Please try again in %d hours.",
                            hoursRemaining));
        }
        ...
        if (!isOtpAllowed(phone, ipAddress)) {
            ...
            if (phone5MinEntry != null && !phone5MinEntry.isExpired()
                    && phone5MinEntry.getCount() >= OTP_MAX_PER_5_MIN) {
                long secondsRemaining = Duration.between(LocalDateTime.now(), phone5MinEntry.getExpiresAt())
                        .getSeconds();
                int minutesRemaining = (int) Math.ceil(secondsRemaining / 60.0);
                throw new RuntimeException(
                        String.format(
                                "Too many OTP requests. Please wait %d minute(s) before requesting another code. You can request up to %d codes every %d minutes.",
    ...
    public void checkAndRecord(String identifier, String action) {
        if (isOtpAction(action)) {
            ...
            if (!isAllowed(identifier, action)) {
                ...
                throw new RuntimeException(
                        String.format("Too many requests. Please wait %d minute(s) before trying again.",
                                GENERAL_WINDOW_MINUTES));
            }
    ...
    /**
     * Check if an action is allowed for the given identifier (general rate
     * limiting)
     */
    ...
    /**
     * Check and record OTP attempt for email - similar to phone but for email
     * identifier.
     */
    public void checkAndRecordEmailOtp(String email, String ipAddress) {
        ...
        if (email5MinEntry != null && !email5MinEntry.isExpired() && email5MinEntry.getCount() >= OTP_MAX_PER_5_MIN) {
            ...
            throw new RuntimeException(
                    String.format("Too many OTP requests. Please wait %d minute(s) before requesting another code.",
                            minutesRemaining));
        }
    ...
    public void cleanup() {
        ...
        // Keep blacklist entries for repeat offense tracking, but clean up very old
        // ones (> 200 hours)
        ipBlacklist.entrySet().removeIf(
                entry -> Duration.between(entry.getValue().getExpiresAt(), LocalDateTime.now()).toHours() > 200);
    }

    /**
     * Check if the action is a phone-based OTP action (not email-based password
     * reset)
    ...
    private boolean isOtpAction(String action) {
        ...
        // PASSWORD_RESET uses email OTP and has its own rate limiting via
        // checkAndRecordEmailOtp
    ...
    private static class IpBlacklistEntry {
        ...
        public boolean hasBeenBlacklistedBefore() {
