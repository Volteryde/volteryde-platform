...
const SPRINGBOOT_API_URL =
  process.env.SPRINGBOOT_API_URL || "http://localhost:8080";
...
export async function updateBookingStatus(
  ...
  console.log(
    `[ACTIVITY] Updating status for booking ${bookingId} to ${status}`,
  );
  ...
  try {
    ...
    console.error(
      `[ACTIVITY] Failed to update status for booking ${bookingId} to ${status}:`,
      error,
    );
...
export async function checkWalletBalance(
  ...
  console.log(
    `[ACTIVITY] Checking wallet balance for user ${userId}. Required: ${requiredAmount}`,
  );
  ...
  try {
    ...
    console.log(
      `[ACTIVITY] User ${userId} balance: ${balance} ${currency}. Sufficient: ${hasSufficientFunds}`,
    );
    ...
    throw new Error(
      `Wallet balance check failed: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
 ...
 */
export async function deductFare(
  userId: string,
  amount: number,
  bookingId: string,
): Promise<WalletTransaction> {
  console.log(
    `[ACTIVITY] Deducting ${amount} from user ${userId} wallet for booking ${bookingId}`,
  );
  ...
  try {
    ...
    if (transaction.status === "SUCCESS") {
      console.log(
        `[ACTIVITY] Wallet deduction successful: ${transaction.transactionId}`,
      );
    } else {
      console.error(
        `[ACTIVITY] Wallet deduction failed: ${transaction.failureReason}`,
      );
    }
    ...
    throw new Error(
      `Wallet deduction failed: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
 ...
 */
export async function refundPartialWalletDeduction(
  transaction: WalletTransaction,
  amount: number,
): Promise<void> {
  console.log(
    `[ACTIVITY] Refunding partial amount ${amount} for transaction ${transaction.transactionId}`,
  );
  ...
  try {
    ...
    throw new Error(
      `Wallet partial refund failed: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
...
export async function reserveSeat(
  ...
  console.log(
    `[ACTIVITY] Route: ${JSON.stringify(request.startLocation)} -> ${JSON.stringify(request.endLocation)}`,
  );
  ...
  try {
    ...
    console.log(
      `[ACTIVITY] Seat reserved: ${reservation.reservationId} on vehicle ${reservation.vehicleId}`,
    );
    return reservation;
  } catch (error) {
...
export async function processPayment(
  ...
  console.log(
    `[ACTIVITY] Processing payment for reservation ${reservation.reservationId}`,
  );
  ...
  try {
    ...
    if (payment.status === "SUCCESS") {
      console.log(
        `[ACTIVITY] Payment successful: ${payment.paymentId} (${payment.amount} ${payment.currency})`,
      );
    } else {
...
export async function confirmBooking(
  ...
  console.log(
    `[ACTIVITY] Confirming booking for reservation ${reservation.reservationId}`,
  );
  ...
  try {
    ...
    console.log(
      `[ACTIVITY] Booking confirmed: ${booking.bookingId} with driver ${booking.driverId}`,
    );
    return booking;
  } catch (error) {
    ...
    throw new Error(
      `Booking confirmation failed: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
...
export async function notifyDriver(
  ...
  console.log(
    `[ACTIVITY] Notifying driver ${notification.driverId} about booking ${notification.bookingId}`,
  );
...
export async function sendNotification(
  ...
  console.log(
    `[ACTIVITY] Sending ${notification.type} notification to user ${notification.userId}`,
  );
  ...
  try {
    ...
    console.log(
      `[ACTIVITY] ${notification.type} notification sent: ${notification.message}`,
    );
  } catch (error) {
 ...
 */
export async function refundWalletDeduction(
  transaction: WalletTransaction,
): Promise<void> {
  console.log(
    `[COMPENSATION] Refunding wallet transaction ${transaction.transactionId}`,
  );
  ...
  try {
    ...
    console.log(
      `[COMPENSATION] Wallet refund completed for user ${transaction.userId}`,
    );
  } catch (error) {
    console.error(`[COMPENSATION] Failed to refund wallet transaction:`, error);
    console.error(
      `[COMPENSATION] Manual refund may be required for transaction ${transaction.transactionId}`,
    );
  }
}
 ...
 */
export async function releaseSeatReservation(
  reservation: Reservation,
): Promise<void> {
  console.log(
    `[COMPENSATION] Releasing seat reservation ${reservation.reservationId}`,
  );
  ...
  try {
    ...
    console.log(
      `[COMPENSATION] Seat ${reservation.seatId} on vehicle ${reservation.vehicleId} released successfully`,
    );
  } catch (error) {
    ...
    console.error(
      `[COMPENSATION] Manual intervention may be required for reservation ${reservation.reservationId}`,
    );
  }
}
...
export async function refundPayment(payment: PaymentDetails): Promise<void> {
  ...
  try {
    ...
    console.log(
      `[COMPENSATION] Refund initiated for ${payment.amount} ${payment.currency}`,
    );
    console.log(
      `[COMPENSATION] Paystack reference: ${payment.paystackReference}`,
    );
  } catch (error) {
    console.error(`[COMPENSATION] Failed to refund payment:`, error);
    // Log but don't throw - we want compensation to be best-effort
    console.error(
      `[COMPENSATION] Manual refund required for payment ${payment.paymentId}`,
    );
  }
}
